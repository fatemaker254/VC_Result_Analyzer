...
SGPA bands (matches the client's reference tables):
  - below_5:    SGPA < 5
  - 5_to_7_5:   5 <= SGPA <= 7.5
  - above_7_5:  SGPA > 7.5
...
This module also builds the cross-department COMPARISON summary (Table 1 /
Table 3 style) once several departments' results have been computed.
"""
...
def analyze_dataframe(df: pd.DataFrame, detection: dict) -> dict:
    ...
    mid_mask = (sgpa_numeric >= 5.0) & (sgpa_numeric <= 7.5)
    above_mask = sgpa_numeric > 7.5
    ...
    metrics = {
        ...
        "sgpa_5_to_7_5": pack(mid_mask),
        "sgpa_above_7_5": pack(above_mask),
    }
...
def build_comparison(departments: list) -> dict:
    """
    departments: list of dicts, each shaped like:
      {
        "department_code": "BOT",
        "department_name": "Botany",
        "total_students": int,
        "metrics": <metrics dict from analyze_dataframe>,
      }

    Returns a dict with:
      - "rows": one row per department (Table 1 + Table 3 columns combined)
      - "aggregate": a totals row across all departments (the "AGGREGATE" row)
      - "top3": the 3 departments with the highest cleared %
    """
    rows = []
    total_appeared = total_cleared = total_not_cleared = 0
    total_below5 = total_mid = total_above = 0

    for d in departments:
        m = d["metrics"]
        appeared = d["total_students"]
        cleared = m["all_cleared"]["count"]
        not_cleared = m["fail"]["count"]
        ...
        row = {
            ...
            "cleared_pct": round(cleared / appeared * 100, 2) if appeared else 0.0,
            "not_cleared_pct": round(not_cleared / appeared * 100, 2) if appeared else 0.0,
            "sgpa_below_5": m["sgpa_below_5"],
            "sgpa_5_to_7_5": m["sgpa_5_to_7_5"],
            "sgpa_above_7_5": m["sgpa_above_7_5"],
        }
        rows.append(row)

        total_appeared += appeared
        total_cleared += cleared
        total_not_cleared += not_cleared
        total_below5 += m["sgpa_below_5"]["count"]
        total_mid += m["sgpa_5_to_7_5"]["count"]
        total_above += m["sgpa_above_7_5"]["count"]

    def _pct(part, whole):
        return round(part / whole * 100, 2) if whole else 0.0

    aggregate = {
        "department_code": "AGGREGATE",
        "department_name": "Aggregate",
        "students_appeared": total_appeared,
        "cleared": total_cleared,
        "not_cleared": total_not_cleared,
        "cleared_pct": _pct(total_cleared, total_appeared),
        "not_cleared_pct": _pct(total_not_cleared, total_appeared),
        "sgpa_below_5": {"count": total_below5, "percent": _pct(total_below5, total_appeared)},
        "sgpa_5_to_7_5": {"count": total_mid, "percent": _pct(total_mid, total_appeared)},
        "sgpa_above_7_5": {"count": total_above, "percent": _pct(total_above, total_appeared)},
    }

    ranked = sorted(rows, key=lambda r: r["cleared_pct"], reverse=True)
    top3 = [
        {
            "department_name": r["department_name"],
            "cleared": r["cleared"],
            "students_appeared": r["students_appeared"],
            "cleared_pct": r["cleared_pct"],
        }
        for r in ranked[:3]
    ]

    return {"rows": rows, "aggregate": aggregate, "top3": top3}
